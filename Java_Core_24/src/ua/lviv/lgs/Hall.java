...
public class Hall implements Comparable<Hall>, Serializable {
	...
	public static Hall inputHall() {
		...
		if (name.equals("")) {
			System.err.println("Theater name entered incorrectly\n");
			name = "--";
		}
	...
	public boolean addTimeTableToHall() throws IllegalTimeFormatException {
		...
		if (isDone) {
			System.out.println("Schedule successfully added to the cinema \"" + name + "\"!\n");
			return true;
		} else
			return false;
	}
	...
	public boolean removeTimeTableFromHall() {
		...
		if (isDone) {
			System.out.println("Schedule has been successfully deleted \"" + name + "\"!\n");
			return true;
		} else
			return false;
	}
	...
	public boolean addScheduleToHall() throws IllegalTimeFormatException {
		...
		if (hallTimeTable.findDayInTimeTable(day).isPresent()) {
			...
			if (isScheduleTimeNonWorkable(day, hallTimeTable, schedule)) {
				...
				System.out.println("Schedule of sessions for " + day.toLiteral(true) + " successfully added \"" + name + "\"!\n");
				return true;
			} else {
				System.err.println("Adding a session schedule to " + day.toLiteral(true) + " to the cinema \"" + name
						+ "\" impossible\n");
				return false;
			}
		} else {
			System.err.println(day.toLiteral(true) + " not on schedule\n");
			return false;
		}
	}
	...
	public boolean removeScheduleFromHall() {
		...
		if (hallScheduleEntryFound.isPresent()) {
			...
			System.out.println("Schedule of sessions for " + day.toLiteral(true) + " deleted successfully \"" + name + "\"!\n");
			return true;
		} else {
			System.err.println(day.toLiteral(true) + " not on schedule\n");
			return false;
	...
	public void viewHallTimeTable() {
		System.out.print("cinema hall \"" + name + "\"\n");
		hallTimeTable.viewTimeTable();
		System.out.println();
	}
	
	public Function<Entry<Days, Schedule>, String> hallScheduleToString() {
		return entry -> "Schedule of sessions in " + entry.getKey().toLiteral(true) + ":\n" + entry.getValue().toString();
	}
